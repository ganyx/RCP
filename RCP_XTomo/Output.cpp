...
{
	upper = Volume_Factor*4.0/3.0*Pi* pow(OuterRadius,3.0);
	lower = Volume_Factor*4.0/3.0*Pi* pow(InnerRadius,3.0);
	if(lower > Density_Roof) Density_Roof = lower;
	printf("%4d, %8d, %10.8f, %10.8f, %5d-%5d, %10.8f\n", 
		*j+1, *i, upper, lower, (*Rods[0].P[0]).number, (*Rods[0].P[1]).number, Density_Roof);
	// Write to Screen.txt
	fprintf(fp, "%4d, %8d, %10.8f, %10.8f, %10.8f\n", *j+1, *i, upper, lower, Density_Roof);
	return;
}
